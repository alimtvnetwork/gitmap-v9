...
8. **Regression-guard is split** (2026-04-29) — `lint-regression-guard` job mixes hard-floor (`unused`, `gosec G115` via `check-lint-regressions.sh`) with baseline-diff (`misspell`, `gocritic`, `exhaustive` via `check-single-linter-diff.sh`). Full JSON diff lives in separate `lint-baseline-diff` job (`lint-diff.py`). When adding a linter, pick one model and document it in the script header. See `.lovable/cicd-issues/02-lint-regression-guard-semantics.md`.
