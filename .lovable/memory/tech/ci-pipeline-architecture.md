...
8. **Hard-floor + baseline-diff are co-located** (2026-04-29, renamed from `lint-regression-guard`) — `lint-hard-floor` job's primary step is hard-floor (`unused`, `gosec G115` via `check-lint-regressions.sh`, `--no-config` so excludes can't mask). The same job also hosts baseline-diff sub-steps (`misspell`, `gocritic`, `exhaustive` via `check-single-linter-diff.sh`) for cache-key locality. Full JSON diff lives in separate `lint-baseline-diff` job (`lint-diff.py`). When adding a linter, pick one model and document it inline at the step. See `.lovable/cicd-issues/02-lint-regression-guard-semantics.md`.
